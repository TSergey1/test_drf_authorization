...
def create_code(length: int = LENGHTH_INVATE_CODE,
    ...
    """Возвращает сгенерированный код состоящий из символов и чисел."""
...
def create_token(length: int = LENGHTH_TOKEN,
    ...
    """Возвращает сгенерированный токен состоящий из чисел."""

    return hashers.make_password(''.join(random.choice(allowed_chars) for _ in range(length)))
